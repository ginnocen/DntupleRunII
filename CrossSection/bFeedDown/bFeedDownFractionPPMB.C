...
TF1* bFeedDownFractionPPMB(Float_t ptMin=2, Float_t ptMax=6)
{
  ...
  TFile* infData = new TFile("/data/dmeson2015/DataDntuple/skim_ncand_D0Dntuple_crab_pp_ALLMinimumBias_AOD_D0_tkpt0p5_Ds_01212016_skimmed.root");
   ...
   TFile* outf = new TFile(Form("outfiles/outputPPMB_%.0f_%.0f.root",ptMin,ptMax),"recreate");
   ...
   TLatex* texCol = new TLatex(0.96,0.93, "PPMB #sqrt{s_{NN}} = 5.02 TeV");
   texCol->SetNDC();
   ...
   cFit->SaveAs(Form("plots/PPMB_%.0f_%.0f_cFit.pdf",ptMin,ptMax));
   ...
   cSideband->SaveAs(Form("plots/PPMB_%.0f_%.0f_cSideband.pdf",ptMin,ptMax));
...
void scan()
{
   ...
   for (int i=20;i<50;i+=5)
     {
       TF1* f = bFeedDownFractionPPMB(i,i+5);
...
int main(int argc, char* argv[])
{
  if(argc==1)
    {
      bFeedDownFractionPPMB();
      return 1;
    }
  else if(argc==3)
    {
      bFeedDownFractionPPMB(atof(argv[1]),atof(argv[2]));
      return 1;
    }
  else
    {
