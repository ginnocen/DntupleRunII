...
int nBins=1;
void testFitUpgrade(TString label="PbPb", int option=2)
{
 ...
 double max;
 TString dada;
 if (option==1){dada="hTest"; rap=1; }
 if (option==2){dada="hTestFake"; rap=2;}

 if (label=="PbPb"){count=3; ptmin=20.; ptmax=30.; max=1200;}
 if (label=="PbPbMB"){count=1;  ptmin=2.; ptmax=3.; max=800000;}


  TCanvas* c= new TCanvas(Form("c%d",count),"",600,600);
  TFile *file=new TFile(Form("foutput_%s.root",label.Data()));
  TH1D* h = (TH1D*)file->Get(dada.Data());
  ...
  tex = new TLatex(0.22,0.83,Form("|y| < %d.0",rap));
  tex->SetNDC();
  tex->SetTextFont(42);
  tex->SetTextSize(0.04);
  tex->SetLineWidth(2);
  ...
  c->SaveAs(Form("%s_%s.pdf",dada.Data(),label.Data()));
