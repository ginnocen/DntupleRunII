void compareMC(int value1=15,int value2=50){
  ...
  TH2F* hemptyPthat=new TH2F("hemptyPthat","",50,value2,100.,10,0.,2.);  
  ...
  TLegendEntry *ent_pthatall=legend->AddEntry(hall,"efficiency sample pthat merged all","pf");
  ent_pthatall->SetTextFont(42);
  ent_pthatall->SetLineColor(4);
  ent_pthatall->SetMarkerColor(4);
  legend->Draw("same");
  c->SaveAs(Form("canvas_phat%d_pthat%d.pdf",value1,value2));
}
