...
TF1* fit(Double_t ptmin, Double_t ptmax)
{
  ...
  if(nBins==1) c->SaveAs(Form("plotFits/DMass_floatwid_%s_inclusive_%.0f_%.0f.pdf",collisionsystem.Data(),ptmin,ptmax));
  else c->SaveAs(Form("plotFits/DMass_floatwid_%s_%.0f_%.0f.pdf",collisionsystem.Data(),ptmin,ptmax));
