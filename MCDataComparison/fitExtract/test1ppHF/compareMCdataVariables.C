...
void compareMCdataVariables(TString promptfile="MCCutVariablePromptPt.root",TString nonpromptfile="MCCutVariableNonPromptPt.root",TString datafile="DataCutVariablePt.root",TString cutvariable="Dpt",TString label="Dpt", double limitlow=0, double limitup=100, TString outputfile="outputPlotPt.root"){
  ...
  hRatio->SetMaximum(2);  
  hRatio->Draw("same");
  canvas->SaveAs(Form("OutPuts/canvasMCDataComp_%s.pdf",label.Data()));
...
int main(int argc, char *argv[])
{
  ...
  if(argc ==9)
    compareMCdataVariables(argv[1],argv[2],argv[3],argv[4],argv[5], atof(argv[6]),atof(argv[7]), argv[8]);
  return 0;
}
