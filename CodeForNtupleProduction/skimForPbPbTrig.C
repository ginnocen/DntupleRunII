...
// main routine
void skimForPbPbTrig(char *infname,char *outfname)
{
   ...
   float Dtrk1Pt[5000];
   float Dtrk2Pt[5000];
   float DsvpvDistance[5000];
   float DsvpvDisErr[5000];
   ...
   for (Long64_t i=0;i<ntDkpi->GetEntries()/1.;i++)
   {
	 ...
	 if (Dsize>0) {
	   ...
	   for (int j=0;j<Dsize;j++) {
	      if (Dtrk1Pt[j]>8&&Dtrk2Pt[j]>8&&DsvpvDistance[j]/DsvpvDisErr[j]>3) {
	         ncand++;
		 break;
	      }
	   }
