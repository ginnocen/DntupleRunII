   ...
   for (Long64_t i=0;i<ntDkpi->GetEntries()/1.;i++)
   {
	 ...
	 if (Dsize>0) {
	   ...
	   for (int j=0;j<Dsize;j++) {
	      if (Dalpha[j]<0.12&&Dpt[j]>0.9&&Dy[j]>-1.1&&Dy[j]<1.1&&(((DsvpvDistance[j]/DsvpvDisErr[j])>5.0&&Dpt[j]<5.0) || ((DsvpvDistance[j]/DsvpvDisErr[j])>3.&&Dpt[j]>5.0))) {
	         ncand++;
		 break;
	      }
	   }
